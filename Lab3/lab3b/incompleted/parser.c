...
void compileProgram(void) {
  ...
  eat(KW_PROGRAM);
  programObj = createProgramObject(lookAhead->string);
  eat(TK_IDENT);
  eat(SB_SEMICOLON);
  enterBlock(programObj->progAttrs->scope);
  compileBlock();
  exitBlock();
  eat(SB_PERIOD);
}
...
void compileBlock(void) {
  ...
  if (lookAhead->tokenType == KW_CONST) {
    ...
    do {
      Object* constObj = createConstantObject(lookAhead->string);
      eat(TK_IDENT);
      eat(SB_EQ);
...
void compileBlock2(void) {
  ...
  if (lookAhead->tokenType == KW_TYPE) {
    ...
    do {
      Object* typeObj = createTypeObject(lookAhead->string);
      eat(TK_IDENT);
      eat(SB_EQ);
...
void compileBlock3(void) {
  ...
  if (lookAhead->tokenType == KW_VAR) {
    ...
    do {
      Object* varObj = createVariableObject(lookAhead->string);
      eat(TK_IDENT);
      eat(SB_COLON);
...
ConstantValue* compileConstant(void) {
  ...
  case SB_MINUS:
    ...
    if (constValue != NULL && constValue->type == TP_INT)
      constValue->intValue = constValue->intValue * sign;
    break;
  case TK_CHAR:
    constValue = makeCharConstant(lookAhead->string[0]);
    eat(TK_CHAR);
...
Type* compileType(void) {
  ...
  case KW_ARRAY:
    ...
    break;
  case TK_IDENT:
    Object* typeObj = lookupObject(lookAhead->string);
    if (typeObj != NULL && typeObj->kind == OBJ_TYPE)
...
void compileParam(void) {
  ...
  switch (lookAhead->tokenType) {
  case TK_IDENT:
    paramObj = createParameterObject(lookAhead->string, PARAM_VALUE, symtab->currentScope->owner);
    eat(TK_IDENT);
    eat(SB_COLON);
    paramObj->paramAttrs->type = compileBasicType();
    declareObject(paramObj);
    break;
  case KW_VAR:
    eat(KW_VAR);
    paramObj = createParameterObject(lookAhead->string, PARAM_REFERENCE, symtab->currentScope->owner);
    eat(TK_IDENT);
    eat(SB_COLON);
    paramObj->paramAttrs->type = compileBasicType();
    declareObject(paramObj);
...
int compile(char *fileName) {
  ...
  if (currentToken != NULL) free(currentToken);
  if (lookAhead != NULL) free(lookAhead);
  closeInputStream();
  return IO_SUCCESS;
