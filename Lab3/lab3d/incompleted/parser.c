...
void compileStatement(void) {
  ...
  case KW_FOR:
    compileForSt();
    break;
    // EmptySt needs to check FOLLOW sets
  case SB_SEMICOLON:
  case KW_END:
  case KW_ELSE:
    break;
...
void compileArguments(ObjectNode* paramList) {
  ...
  case SB_LPAR:
    ...
    if (paramList != NULL && paramList->next != NULL)
      error(ERR_PARAMETERS_ARGUMENTS_INCONSISTENCY, currentToken->lineNo, currentToken->colNo);
    eat(SB_RPAR);
    break;
    // Check FOLLOW set 
  ...
  case KW_THEN:
    ...
    // FIXED: Check if paramList is NULL before accessing paramList->object
